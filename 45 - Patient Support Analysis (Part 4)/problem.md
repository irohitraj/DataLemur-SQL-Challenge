# Long-Calls Month-Over-Month Growth [SQL Interview Question]
...
- **Call counts:**
  - Jan: 1 call (1 long-call).
  - Feb: 0 calls.
  - Mar: 1 call (1 long-call).
  - Apr: 3 calls (3 long-calls).
  - May: 2 calls (2 long-calls).
  - Jun: 2 calls (2 long-calls).

- **Growth percentage calculations:**
  - **January**: No prior data, so growth is `NULL`.
  - **February**: No calls in February, so growth from January is `0.0%`.
  - **March**: 1 call in March (compared to 0 in February), so growth is `100.0%`.
  - **April**: 3 calls in April (compared to 1 in March), so growth is `200.0%`.
  - **May**: 2 calls in May (compared to 3 in April), so growth is `-33.3%`.
  - **June**: 2 calls in June (same as May), so growth is `0.0%`.
